...
<form on:change on:input class="vec-row" {style}>
  <RangeSlider bind:value={value.x} label="x" type="RAD" {min} {max} {step} style="display: block;"/>
  <RangeSlider bind:value={value.y} label="y" type="RAD" {min} {max} {step} style="display: block;"/>
  <RangeSlider bind:value={value.z} label="z" type="RAD" {min} {max} {step} style="display: block;"/>
</form>
